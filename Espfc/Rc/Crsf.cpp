...
{
  // reserve 3 bytes for the ext header (dest, origin, status) written before the data;
  // the crc occupies the extra slot in payload[CRSF_PAYLOAD_SIZE_MAX + 1]
  auto fragmentEnd = std::min(begin + CRSF_PAYLOAD_SIZE_MAX - 3, end);
...
static inline void fillMessage(const CrsfMessage& frame, Connect::MspMessage& m, Connect::MspVersion version)
{
  ...
  // Reject frames too short to hold the MSP header, otherwise the unsigned
  // framePayloadSize below would underflow and cause an out-of-bounds read.
  if(frame.size < 5 + sizeof(HeaderType))
  {
    return;
  }
