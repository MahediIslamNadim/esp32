...
Pid::Pid():
  rate(1.0f), dt(1.0f), Kp(0.1f), Ki(0.f), Kd(0.f), Kf(0.0f),
...
float FAST_CODE_ATTR Pid::update(float setpoint, float measurement)
{
  ...
  const float output = pTerm + iTerm + dTerm + fTerm;
  // std::clamp does not filter NaN/Inf (NaN compares false), so guard explicitly.
  // A bad sensor sample must never reach the mixer; also reset the integrator so
  // a poisoned iTerm cannot persist across iterations.
  ...
  return std::clamp(output, oLimitLow, oLimitHigh);
}
