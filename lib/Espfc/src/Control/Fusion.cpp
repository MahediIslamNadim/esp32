...
int Fusion::begin()
{
  ...
  _ekf.begin(_model.state.accel.timer.rate);
  _ekf.setKp(_model.config.fusion.gain * 0.05f);
...
void Fusion::restoreGain()
{
  ...
  _ekf.setKp(_model.config.fusion.gain * 0.02f);
}
...
int FAST_CODE_ATTR Fusion::update()
{
  ...
  if(_model.accelActive())
  {
    switch(_model.config.fusion.mode)
    {
      ...
      // Sanitize: a single bad sensor sample can produce a NaN/Inf that would
      // otherwise poison attitude permanently and propagate into the PID/motors.
      if(!_model.state.attitude.quaternion.isFinite())
      {
        _model.state.attitude.quaternion = Quaternion();
      }
...
void Fusion::experimentalFusion()
{
  ...
  Quaternion r = Quaternion::lerp(Quaternion(), _model.state.accel.adc.accelToQuaternion(), 0.5f);
...
void Fusion::simpleFusion()
{
  ...
  if(_model.state.attitude.euler.z > (float)PI) _model.state.attitude.euler.z -= (float)TWO_PI;
  if(_model.state.attitude.euler.z < -(float)PI) _model.state.attitude.euler.z += (float)TWO_PI;
}
...
void Fusion::complementaryFusion()
{
  ...
  for(size_t i = 0; i < 3; i++)
  {
    ...
    if(angle > (float)PI) angle -= (float)TWO_PI;
    if(angle < -(float)PI) angle += (float)TWO_PI;
    _model.state.attitude.euler.set(i, angle);
  }
  _model.state.attitude.quaternion = _model.state.attitude.euler.eulerToQuaternion();
...
void FAST_CODE_ATTR Fusion::ekfFusion()
{
  _ekf.update(
    _model.state.attitude.rate.x, _model.state.attitude.rate.y, _model.state.attitude.rate.z,
    _model.state.accel.adc.x, _model.state.accel.adc.y, _model.state.accel.adc.z
  );
  _model.state.attitude.quaternion = _ekf.getQuaternion();
  _model.state.attitude.euler  = _ekf.getEuler();
}
