...
class BlackboxSerialBuffer: public Device::SerialDevice
{
  public:
    ...
    // Owns a raw buffer; non-copyable to avoid double-free (rule of 3)
    BlackboxSerialBuffer(const BlackboxSerialBuffer&) = delete;
    BlackboxSerialBuffer& operator=(const BlackboxSerialBuffer&) = delete;
    ...
    {
      _dev = s;
      if(!_data) _data = new uint8_t[SIZE]; // guard against leak on repeated wrap()
    }
    ...
    {
      if(!_data) return 0; // not wrapped yet; avoid null dereference
      _data[_idx++] = c;
      if(_idx >= SIZE) flush();
      return 1;
    }
